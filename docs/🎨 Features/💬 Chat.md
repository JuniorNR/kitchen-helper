# 💬 Chat Feature

Функциональность чата для обмена сообщениями между пользователями.

## 📍 Расположение

`src/features/Chat/`

## 📂 Структура

```
Chat/
├── index.ts                    # Публичный API
├── model/
│   ├── chat.types.ts          # Типы компонентов чата
│   └── chat.utils.ts          # Утилиты (константы)
└── ui/
    ├── Chat.tsx               # Главный компонент чата
    ├── ChatListAside.tsx      # Боковая панель со списком чатов
    ├── ChatMessagesWindow.tsx # Окно сообщений
    ├── ChatSendControlPanel.tsx # Панель отправки сообщений
    ├── ChatListAside.skeleton.tsx # Skeleton загрузки списка
    ├── ChatMessagesWindow.skeleton.tsx # Skeleton загрузки сообщений
    └── chat.module.scss       # Стили чата
```

## 📄 Компоненты

### [[💬 Chat Component|Chat.tsx]]

Главный компонент чата, объединяющий все части.

**Функциональность**:
- Управление активным чатом
- Загрузка сообщений
- Real-time обновления через Laravel Echo
- Пагинация старых сообщений
- Сохранение активного чата в localStorage

**Связанные документы**:
- [[💬 Chat Entity|📦 Entities/💬 Chat]]

---

### [[📋 ChatListAside|ChatListAside.tsx]]

Боковая панель со списком чатов.

**Функциональность**:
- Отображение списка чатов
- Выделение активного чата
- Показ последнего сообщения
- Индикация непрочитанных сообщений
- Анимации при переключении

**Особенности**:
- Адаптивный дизайн
- Поддержка темной темы
- Визуальное различие своих и чужих сообщений

---

### [[💬 ChatMessagesWindow|ChatMessagesWindow.tsx]]

Окно для отображения сообщений.

**Функциональность**:
- Отображение сообщений в обратном порядке
- Бесконечная прокрутка вверх для загрузки старых сообщений
- Автоматическая прокрутка к новым сообщениям
- Разделение своих и чужих сообщений
- Анимации появления сообщений

**Особенности**:
- Infinite scroll для пагинации
- Автоматический скролл при смене чата
- Отображение времени сообщений

---

### [[📤 ChatSendControlPanel|ChatSendControlPanel.tsx]]

Панель для отправки сообщений.

**Функциональность**:
- Ввод текста сообщения
- Отправка сообщения
- Индикация загрузки отправки
- Очистка поля после отправки

---

## 🔧 Утилиты

### `chat.utils.ts`

Константы для чата:
- `LIMITS_MESSAGES` - лимит сообщений на страницу (25)

## 🔄 Real-time обновления

Чат использует Laravel Echo для real-time обновлений:
- Событие `.MessageSent` - новое сообщение
- Событие `.ChatUpdated` - обновление чата

## 🔗 Связанные документы

- [[💬 Chat Entity|📦 Entities/💬 Chat]]
- [[🔧 Shared/lib/providers/EchoProvider|Echo Provider]]
- [[🔧 Shared/lib/configs/echo|Echo конфигурация]]

