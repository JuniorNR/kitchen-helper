				...
				<DropdownMenu>
					{NAV_ITEMS.map((item) => {
						...
						return (
							...
							>
								{item.href === '/market' ? (
									<span className="p-2 block opacity-60 cursor-not-allowed">
										{tCommon(item.labelKey)} (work in progress)
									</span>
								) : (
									<Link
										prefetch
					...
					return (
						...
						>
							{item.href === '/market' ? (
								<span
									className={
										'px-3 py-1 rounded-full outline-none opacity-60 cursor-not-allowed'
									}
								>
									{tCommon(item.labelKey)} (work in progress)
								</span>
